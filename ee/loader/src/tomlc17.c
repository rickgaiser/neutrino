...
static int read_time(const char *p, int *hour, int *minute, int *second,
  ...
  *usec = 0;
  if (3 == sscanf(p, "%2d:%2d:%2d%n", hour, minute, second, &n) && n == 8) {
    ...
    while (isdigit((uint8_t)*p) && micro_factor) {
...
static int scan_timestamp(scanner_t *sp, token_t *tok) {
  ...
  int lineno = sp->lineno;
  const char *p = buffer;
  if (isdigit((uint8_t)p[0]) && isdigit((uint8_t)p[1]) && p[2] == ':') {
  ...
  toktyp = TOK_DATE;
  p += n;
  if (!((p[0] == 'T' || p[0] == ' ' || p[0] == 't') && isdigit((uint8_t)p[1]) &&
        isdigit((uint8_t)p[2]) && p[3] == ':')) {
    goto done; // date only
  }
...
static int process_numstr(char *buffer, int base, const char **reason) {
  ...
  for (int i = 0; buffer[i]; i++) {
    if (buffer[i] == '.') {
      if (i == 0 || !isdigit((uint8_t)buffer[i - 1]) || !isdigit((uint8_t)buffer[i + 1])) {
...
static inline bool test_time(const char *p, const char *endp) {
  return &p[2] < endp && isdigit((uint8_t)p[0]) && isdigit((uint8_t)p[1]) && p[2] == ':';
}

// Check if the next token may be DATE
static inline bool test_date(const char *p, const char *endp) {
  return &p[4] < endp && isdigit((uint8_t)p[0]) && isdigit((uint8_t)p[1]) && isdigit((uint8_t)p[2]) &&
         isdigit((uint8_t)p[3]) && p[4] == '-';
}
...
static int scan_literal(scanner_t *sp, token_t *tok) {
  *tok = mktoken(sp, TOK_LIT);
  const char *p = sp->cur;
  while (p < sp->endp && (isalnum((uint8_t)*p) || *p == '_' || *p == '-')) {
    p++;
  }
