...
/* Frame offsets (from Ethernet frame start):
 *   0x00: ETH header (14 bytes)
 *   0x0E: IP  header (20 bytes)
 *   0x22: UDP header  (8 bytes)
 *   0x2A: UDP payload — UDPBD header (2 bytes, read as padded u32 starting at 0x28)
 *   0x2C: UDPBD payload (info/rdma/done data)
 */
static inline void _cmd_info_reply(struct SUDPBDv2_Header *hdr)
{
    if (bdm_connected == 0)
    {
        smap_fifo_read(0x2C, &g_udpbd.sectorSize,  4);
        smap_fifo_read(0x30, &g_udpbd.sectorCount, 4);
        bdm_connected = 1;
...
static inline void _cmd_read_rdma(struct SUDPBDv2_Header *hdr)
{
    ...
    // DMA payload directly into the user buffer
    // smap_fifo_read() handles block-size limits internally
    smap_fifo_read(0x30, g_buffer_act, size);
